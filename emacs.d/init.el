...
(defhydra hydra-calc (:color blue :hint nil)
  ...
  ----------------------------------------------------------------------
  ※ 全設定 of the calculator (full reset) is [C-x * 0].
  ----------------------------------------------------------------------
  [_p_] メインメニューに戻る            [_q_] 閉じる
"
  ("c" calc)
...
(use-package calfw
  ...
  :config
  ;; 見栄え of the calendar layout (grid lines, etc.)
...
(use-package visual-replace
  ...
  (setq visual-replace-default-to-full-scope t) ; デフォルトはファイル全体にする

  ;; 正規表現ありで起動するカスタムコマンドを定義
  (defun my/visual-replace-regexp ()
    "正規表現モードで visual-replace を起動します。"
    (interactive)
  ...
  ;; C-M-% で正規表現ありの visual-replace を起動
  (global-set-key (kbd "C-M-%") 'my/visual-replace-regexp)
