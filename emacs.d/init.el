...
(add-hook 'window-setup-hook
          (lambda ()
            ...
            (my/tb-sep '16-sep)
            ;; カラーマーカー
            (my/tb-add '15-marker-clr 'my/marker-remove-all            "tb-marker-clear" "全マーカーを削除")
            (my/tb-add '14-marker-put 'my/marker-put                   "tb-marker"       "カラーマーカー付け/消し")
            (my/tb-sep '13-sep)
            ;; 検索
            (my/tb-add '12b-filter    'my/emeditor-filter               "tb-filter"       "フィルタ（マッチ行のみ表示・直接編集）")
...
(defhydra hydra-calc (:color blue :hint nil)
  ...
  [_m_] Casual メニューを開く             [_r_] RPN モード ON  (スタック式)
  ----------------------------------------------------------------------
  [_0_] スタック全消去 (AC)
  ----------------------------------------------------------------------
  [_p_] メインメニューに戻る            [_q_] 閉じる
...
(defhydra hydra-marker (:color blue :hint nil)
  ...
  [_u_] 全マーカーを削除                [_p_] 前のマーカーへ
  [_c_] Casual メニュー（マーカー上で） [_d_] 定義箇所へジャンプ
  ----------------------------------------------------------------------
  [_b_] メインメニューに戻る            [_q_] 閉じる
"
  ("m" my/marker-put)
...
(with-eval-after-load 'hydra
  (let ((menu-map (make-sparse-keymap "Navigator")))
    ;; Navigator メニュー内の項目
    (define-key menu-map [hydra-everything] '(menu-item "Everything PC内検索 (M-o e)" consult-locate))
    (define-key menu-map [hydra-fd-project] '(menu-item "ファイル名検索 fd (M-o g)" my/consult-fd-project))
    (define-key menu-map [hydra-fd-here]    '(menu-item "ファイル名検索 fd ここから (M-o G)" my/consult-fd-here))
    (define-key menu-map [separator-1]      '(menu-item "--"))
    (define-key menu-map [hydra-epub]       '(menu-item "EPUB リーダー (M-o P)" my/nov-open-epub))
    (define-key menu-map [hydra-new-frame]  '(menu-item "新しいウィンドウを開く (M-o n)" make-frame))
    (define-key menu-map [separator-2]      '(menu-item "--"))
    (define-key menu-map [hydra-marker]     '(menu-item "カラーマーカー (M-o M)" hydra-marker/body))
    (define-key menu-map [hydra-calc]       '(menu-item "電卓 (M-o C)" hydra-calc/body))
    (define-key menu-map [hydra-calendar]   '(menu-item "カレンダー (M-o L)" my/open-calendar))
    (define-key menu-map [hydra-file]       '(menu-item "ファイル操作 (M-o F)" hydra-file/body))
    (define-key menu-map [hydra-window]     '(menu-item "ウィンドウ操作 (M-o w)" hydra-window/body))
    (define-key menu-map [separator-3]      '(menu-item "--"))
    (define-key menu-map [hydra-obsidian]   '(menu-item "Obsidian メニュー (M-o o)" hydra-obsidian/body))
    (define-key menu-map [hydra-markdown]   '(menu-item "Markdown メニュー (M-o m)" hydra-markdown/body))
    (define-key menu-map [separator-4]      '(menu-item "--"))
    (define-key menu-map [hydra-main]       '(menu-item "メインランチャーを開く (M-o)" hydra-launcher/body))
...
(use-package markdown-mode
  ...
  (defun my-markdown-toolbar-setup ()
    (setq-local tool-bar-map (make-sparse-keymap))
    (tool-bar-local-item "insert"      'markdown-insert-header-1       'md-h1     tool-bar-map :help "見出し1")
    (tool-bar-local-item "go-forward"  'markdown-insert-header-2       'md-h2     tool-bar-map :help "見出し2")
    (tool-bar-local-item "preferences" 'markdown-insert-bold           'md-bold   tool-bar-map :help "太字")
    (tool-bar-local-item "describe"    'markdown-insert-italic         'md-italic tool-bar-map :help "斜体")
    (tool-bar-local-item "index"       'markdown-insert-gfm-code-block 'md-code   tool-bar-map :help "コードブロック")
    (tool-bar-local-item "jump-to"     'markdown-insert-link           'md-link   tool-bar-map :help "リンク"))
  (add-hook 'markdown-mode-hook 'my-markdown-toolbar-setup))
...
(defun my/open-calendar ()
  ...
  (cfw:open-calendar-buffer
   :contents-sources
   (list
    (cfw:howm-create-source "SkyBlue") ; howm の予定
    (cfw:org-create-source  "Orange")  ; org の予定
    )))
...
;; =====================================================================
;; Emacs 自動生成の設定欄（custom-set-variables）
;; ※ここより下は Emacs が自動で書き換えます。手動編集は避けてください。
;; =====================================================================
...
(custom-set-variables
 ...
 '(custom-safe-themes
   '("dc8d63cd0514d058bdf34774163709834519350c1a80dc04deaa58085a8810a5"
     ...
     "3f64893685082a6af2c99ba3bfe29238d5e6ad7ba03ec738b7ae439531995016"
     "6ac71d4a6f5909204adf102fa64dfc1647b8db90d369d02b466635cc52d18fb0"
     ...
     "2d16738941db41d3bf9b293b7afbcefc4adcf15d4baef6ac95a9185724bea3ba"
     "917ce3095099ba4757f60ed7a4ece1b26c8d820afe33a3b578cc46c1dedbff08"
     "b42f29befd2a39bf1f246a7c111bf226588988901c18b3d4de18eb565a545ba0"
     "48b54c5eb3a9ac1d81ffec4c9f6aaf68511dc0b601b256b62483f390c0219dc7"
     ...
     "dc4ef9f45ff89b6e39b6949d7e8529dd78ddcb766dc3a7122498b6b43dafe808"
     "56e7eb6ba5dbe81bad46f22f6eb74682caf69dc652e57f1538ea52de93d44289"
     "05dd3c709da9cdcdeebe212f427866952a2c604688b0213ae6cbc8b306becc22"
     "cd0be7158ada1340c046ba259a69130b753ee39d3ebbdfb2a1157af4ae8a1f17"
     "bec534832002024e80b4cdc8095b65b0c7772778e27be379922c55af399de3fd"
     "379dfdfb8d81433068f8b472745eb9f5e79a651fd79909e4865b9fd718b0d008"
     ...
     "f9f7fe512eedf7f0054755a39fbbc1b166789fd1ee89e944bbcb6fb2f930b7a5"
     "b27fe4349491601643abd64299ee901f9381bd6cc962c2588b3f272c34bebec0"
     ...
     "6f215350ccf35ea64956e0b87111dc43e4cfb0ce0ea28f8010918d767c3e4e9a"
     "696532e9091ad69d9b7fc01ad176826ab9b175285a6801ce227760f0ab68c842"
     "9b367ddf7dac200dc3ff9994ef1add6f5cf4f332566415b4d54d8bc7d0d57fd8"
     "ee3eb912d37c6628d2ffac29f39d9165f5f5aff101a6f60fe01893387a0cb874"
     "ae228d52d174b134798e5b28ba9513f454c1c92943afb7e9e1f93e24544bf05a"
     "b54b7032974d55884e2107ea5e99c8b0bb16515e643a0e5d367026a34e973646"
     "aeec1dc9460220d6f10fdb2870c39556fc05d7ffe11ee4283722f04a93a51998"
     "1442e9746e5e7c8291d9f7dbf60b69a68631079a74e8b24edfd3008cc1ad276f"
     "2e35cbad10e9e5c379fcd083ad227f6450a06d567ac7d258b862b36c41f7f256"
     "b384aa2409c8f69c5294f151b8e77c1bcf3f52da076f3c706848088c27d541e9"
     "3bc4c852a03c439cb78a044b55228749c1a03ce01a728500339749f4a7b6f0c1"
     "e1fd72b4cebe772a10425c2d9cda97150b8d449adcfe293afeb5d81926bd21c4"
     ...
     "7fcf9978d9cabe73109eecbbcdc488ae953cac9e5aaa4e9c8f50f61662c74864"
     "c3b79f9c858c509d2f38529ec230ef981e4e1bb7042027fa89ad2528be9d4b7b"
     ...
     "477e5e132dc3ca232c613e390f04dc3f66aead8cf34a90f6955fcec7ab98108e"
     ...
     "9d6db28e1463ffd1bba96456e37c80bbe78d118749b0b319fa8b21bd0be60120"
     ...
     "f6fb035d75c77f7f33b9255cafc7e313b5377bd5c7bba1b7f77f7cecfa0d97ba"
     "85024dad36e7fc17103087047990013f36ac6d3081f0ff73692852abf6bba943"
     "4b93ef0ed10715e0d3c3bdf735db394ca098df92f1bd68afde1662515e3b5a17"
     "e52261fa9b93e0d058393b933595971085e9c25e5507fbb2cb4b441ecef3c1db"
     "f8a254cf3ffdc2989d79313c72620e81ba3f9d976915fa34ebffcd8e9cc804b5"
     "2b4b734833f7f1b59a4c0040989e9fa1ef8ee9df39ce746da981a7b4933e71bc"
     "5bb3cbe6acae048708aec99b8ce152a5cd29db5b8723f9900136ca3877c37b39"
     "09354e8cdee4d7f0fe0c0ad817c41324f36dc7d093f0f72f57969e55977764e0"
     "4ba4e0423607b82429ec0bd4873f484766370a944559564ba78220f060604a43"
     "0f24d8b040c4d99f364c1f4bddb1402cc327d2c7f4151d369401aaf8d25cd5f8"
     "ad8c549b7a99f6e8d24355baf57efdbb273aaf24ebf5c011bace16bc45f93088"
     "d63097e5458242ae1f4ea85a4000f126f5112789c5e4c28bad0086f74d1d387e"
     "7e0f3481838a0edf460d11e44c2bbc5f4cda85a6528caa9737e49db4794206d7"
     "f4f8ac538fb5df1b706a5a96254afba08982ebb63a6ad05f633e44e0498afbde"
     "89b9c7589d34ca00c01e4a5cacef56c95a36705903dfde9706301517c1f65874"
     ...
     "f673fcb252373cc618097e29647325c7afa0bc586a34588f8a6603651ba3f789"
     "e8b6c9c9372f2e3a7fed3887497987dacf9952a75681c3aeabbbad044499e067"
     "bdf76a886a40d7bf014d2a5453acfff5c5d1ce25c39e9d011f7bf5c9adc85b17"
 ...
 '(package-selected-packages
   '(acp atomic-chrome autothemer beacon bicycle calfw-howm calfw-org
	 ...
	 nerd-icons nov nyan-mode obsidian orderless org-download
	 persist persistent-scratch rainbow-delimiters smartparens
	 taxy-magit-section thrift tr-ime track-changes
	 vertico-posframe visual-regexp vscode-dark-plus-theme wgrep
	 zoxide))
...
(defun my/emeditor-context-menu (menu click)
  ...
  (define-key-after menu [my-popup-search]
    `(menu-item "バッファ内検索 (popup-search)  [C-c l]"
  ...
  (let ((conv-map (make-sparse-keymap "変換")))
    ...
    (define-key conv-map [calc-spot]
      '(menu-item "数式をその場で計算 (calc-onthespot)" my/ctx-calc-onthespot
