...
日本語環境・ポータブル運用を前提に、テキスト編集や個人メモ管理、AIツール連携などを快適に行えるよう構成しています。

> **設計思想**: 一般的なキーボード完結型の Emacs 設定とは異なり、**GUI・マウス操作を併用する Windows エディタらしい使い心地**を目指しています。
> メニューバー、ツールバー、タブバー、右クリックメニューを活かし、一般的な Windows アプリと同じ `Ctrl+C/V/Z`（CUA キーバインド）で直感的に操作できるようにしています。
> 同時に、キーボードで素早く編集したい場面にも対応できるよう **Meow（モーダル編集）** を組み込み、CtrlキーはWindows操作、通常キーはMeowという形で無理なく共存させています。
...
- **オンデマンド読み込みによる起動最適化** — 多数のパッケージを導入しつつも起動がもたつかないよう、Obsidian や howm などの重い処理は使用時まで遅延ロードさせています。ダッシュボード上には読み込みパッケージ数と起動時間を表示します
...
- **日本語対応** — Migemo によるローマ字検索、cp932 ファイルパス対応、mozc-modeless によるモードレス日本語入力を含みます。Windows Terminal などの `-nw`（ターミナル）環境下でも、OS の IME 制約を回避して `Ctrl+\` でスムーズに日本語を入力できます
...
| 21b. Mozc 日本語入力 | mozc-modeless による日本語入力、`C-\\` で手動 ON/OFF トグル。Windows Terminal 等の `-nw`（CUI）環境とも相性が良く、ターミナル内でも安定して日本語入力が可能 |
...
- **OS**: Windows 10 / 11
- **Emacs**: GNU Emacs 29 以上
- **日本語入力（※ mozc-modeless を使う場合）**:
  - Windows に本家 **「Google 日本語入力」** がインストールされていること（※ `mozkey` などの個別・派生ツールではなく、公式の Google 日本語入力本体が必要です）。
  - **常用する既定の IME に設定しておく必要はありません**（Windows 全体は Microsoft IME や ATOK のままで構いません。Emacs が裏で変換サーバーとのみ自動通信します）。
