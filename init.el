...
(use-package meow
  ...
  (defun my/meow-select-matches-in-region (pattern)
    "選択範囲内のPATTERNにマッチする箇所すべてをBeacon（マルチ選択）にする。
スマートケース対応: 小文字のみなら大文字小文字を無視し、大文字を含めば厳密一致。"
    (interactive "sMatch in selection: ")
    (unless (use-region-p)
      ;; 選択範囲がない場合はカーソル下の単語を選択
      (meow-mark-word 1))
...
あったため、キー自体をこの関数に差し替えて実行順序を確定させる。"
  ...
  ESC : NORMAL復帰 (IME OFF)  C-c / C-x : コピー / 切取 (M-0〜9可)  C-e : 行末/インデント/行頭
                              C-v / C-z : 貼付 (M-0〜9可) / Undo    Shift+矢印 : 範囲選択
...
"
  ...
  ----------------------------------------------------------------------
  [Tab / i] INSERTへ   [C-]] 強制脱出 (非常口)   [H] 標準ヘルプ   [q / ESC / F1] 閉じる
"
